...
class MateriasShow extends Component
{
    ...
    {   
        #$asignatura = null;
        #$this->emit("Mostrar", "ES:  ". json_encode($this->getGradoNombraminetoDocente()));
        return view('livewire.administracion-docente.materias-show', ['inscritos' =>  $this->getDataNotas(), 'nombramiento' => $this->getGradoNombraminetoDocente()]);  
    }
    ...
    private function getGradoNombraminetoDocente(){
        /* return SiadiPlanificarAsignatura::join('base_upea.vista_persona as vp', 'siadi_planificar_asignaturas.id_asignacion_docente', '=', 'vp.id')
            ...
            ->select(
                'vp.ci', 
                \DB::raw("CONCAT(vp.paterno, ' ', vp.materno, ' ', vp.nombre) AS nombre_docente"), 
                'vacda.sede', 
                'vacda.item_nombramiento')
            ->where('siadi_planificar_asignaturas.id_planificar_asignatura', '=', $this->asignatura->id_planificar_asignatura)
            ->where('vp.id', '=',  $this->asignatura->$id_asignacion_docente)
            #->where('vacda.carsed_id', '=', $sede) # ->where($sede, 'LIKE', \DB::raw("CONCAT('%', vacda.sede, '%')"))
            ->where('vacda.item_nombramiento', 'LIKE', \DB::raw("'%IDI%'"))
            ->first(); */
        return tabla_vista_asignacion_control_docente_actua::
    ...
    public function edicion_notas(){
        ...
        $this->emit("deshabilitarRuedasMouse", '.notas-finales');
    ...
    public function guardar_notas(){
        ...
        if($this->operation=="editar_notas"){
                ...
                foreach($datos_inscripcion as $indice => $valor){
                    if($this->observacion_notas[$indice]!=='BAJA'){
                        if($this->id_notas[$indice]==$valor->id_nota && strval($valor->final_nota)!==strval($this->final_notas[$indice]) || $this->observacion_notas[$indice]!== $valor->observacion_nota){
                            ...
                            $nota->observaciones_detalle = $this->observacion_notas[$indice] .' '. $this->asignatura->siadi_convocatoria->modalidad->nombre_convocatoria_estudiante . $this->asignatura->siadi_convocatoria->id_modalidad_curso==1? ' ('. $this->asignatura->siadi_convocatoria->convocatoria_costo->tipo_costo .')': '';
            ...
            $this->emit("Mostrar", "La operacion no es valida");
        }
    }

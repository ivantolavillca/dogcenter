        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-3 row">
                        <div class="col-md-6">
                            <button class="btn btn-outline-primary waves-effect waves-light col-md-6 " wire:click="crear_publicacion"> 
                            ...
                            @foreach ($publicaciones as $pu)
                                <div class="card mb-4 col-md-4">
                                    <div class="card-title text-center bg-success bg-gradient text-white">
                                        <div class="float-end bg-white text-secondary m-0 p-1 border border-1 border-success" title="Modificado por"><b>{{$pu->usuario->iniciales}}</b></div> 
                                    ...
                                    <div class="ratio ratio-16x9">
                                        ...
                                        @elseif($pu->publicaciones_tipo=='Video')
                                            <div class="mb-3 col-lg-12 ratio ratio-16x9">
                                                ...
                                                src="https://www.youtube.com/embed/{{$pu->publicaciones_imagen_url}}?disablekb=1&modestbranding=1"
                                                frameborder="0" allowfullscreen>
                                                </iframe> {{-- M7lc1UVf-VE --}}
                                            </div>
                                        @else {{-- comunicado - convocatoria --}}
                                            <img class="card-img-top img-fluid " src="{{ $pu->publicaciones_imagen_url }}" alt="Image {{ $pu->publicaciones_titulo }}">
                                        @endif
                                    </div>
                                    <div class="card-body">
                                        <h4 class="card-title mt-0">{{ $pu->publicaciones_titulo }}</h4>
                                        <p class="card-text">{{ $pu->publicaciones_descripcion }}.</p>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3 col-lg-12  ">
                        ...
                        <select class="form-select @error('tipopublicacion') border-danger @enderror" wire:model="tipopublicacion" id="tipopublicacion">
                    ...
                    <div class="mb-3 col-lg-12">
                        ...
                        <textarea id="descripcion" cols="30" rows="8" class="form-control @error('descripcion') border-danger @enderror" wire:model="descripcion"></textarea>
                        @error('descripcion')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                        ...
                        <div class="mb-3 col-lg-12">
                            <label class="form-label">IMAGEN: 
                                @if($tipopublicacion == 'Convocatoria')
                                    <span class="text-secondary">resolución recomendada en px (<span title="Ancho">{{$CONVOCATORIA_PX_ANCHO}}</span>x<span title="Alto">{{$CONVOCATORIA_PX_ALTO}}</span>)</span>
                                @endif
                                @if($tipopublicacion == 'Comunicados')
                                    <span class="text-secondary">resolución recomendada en px (<span title="Ancho">{{$COMUNICADO_PX_ANCHO}}</span>x<span title="Alto">{{$COMUNICADO_PX_ALTO}}</span>)</span>
                                @endif

                                @if($tipopublicacion == 'Convocatoria' || $tipopublicacion == 'Comunicados')
                                    <br><small class="text-info">Si la imagen es mayor se redimensionara a la resolución recomendada.</small>
                                @endif
                            </label>
                            <input type="file" class="form-control @error('imagen') border-danger @enderror" wire:model="imagen"
                    ...
                    @if ($tipopublicacion == 'Horario')
                        <div class="mb-3 col-lg-12">
                            ...
                            <input type="file" class="form-control @error('imagen') border-danger @enderror" wire:model="imagen" accept=".pdf">
                            @error('imagen')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    @elseif($tipopublicacion == 'Convocatoria')
                        <div class="mb-3 col-lg-6">
                            <label class="form-label" for="fecha_inicio_convocatoria">FECHA INICIO DE INSCRIPCIONES CONVOCATORIA</label>
                            <input type="date" class="form-control @error('fecha_inicio_convocatoria') border-danger @enderror" wire:model="fecha_inicio_convocatoria" id="fecha_inicio_convocatoria">
                            @error('fecha_inicio_convocatoria')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="mb-3 col-lg-6">
                            <label class="form-label" for="fecha_fin_convocatoria">CIERRE DE INSCRIPCIONES CONVOCATORIA:</label>
                            <input type="date" class="form-control @error('fecha_fin_convocatoria') border-danger @enderror" wire:model="fecha_fin_convocatoria" id="fecha_fin_convocatoria">
                            @error('fecha_fin_convocatoria')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    @elseif($tipopublicacion == 'Video')
                        <div class="mb-3 col-lg-12">
                            <label for="url_video">ID URL VIDEO YOU TUBE</label>
                            <input type="text" class="form-control @error('url_video') border-danger @enderror" wire:model="url_video" id="url_video">
                            @error('url_video')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        @if(!is_null($url_video) && strlen($url_video)>=3)
                        <div class="mb-3 col-lg-12 ratio ratio-16x9">
                            <iframe  type="text/html" class=""
                            src="https://www.youtube.com/embed/{{$url_video}}?disablekb=1&modestbranding=1"
                            frameborder="0" allowfullscreen>
                            </iframe> {{-- M7lc1UVf-VE --}}
                        </div>
                        @endif
                        <p><a target="_blank" href="https://soporte.easypromosapp.com/hc/es/articles/360043822672-V%C3%ADdeo-C%C3%B3mo-obtener-el-ID-de-un-v%C3%ADdeo-de-YouTube" class="text-info">¿Cómo obtener id de un video?</a></p>
                    @endif
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input type="text" class="form-control @error('tipopublicacion') border-danger @enderror" wire:model="tipo_publicacion_edit" disabled style="color: black">
                        @error('tipopublicacion')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3 col-lg-12">
                        ...
                        <input type="text" class="form-control @error('titulo_edit') border-danger @enderror" wire:model="titulo_edit">
                        @error('titulo_edit')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mb-3 col-lg-12">
                        ...
                        <textarea name="" id="" cols="30" rows="10" class="form-control @error('descripcion_edit') border-danger @enderror" wire:model="descripcion_edit"></textarea>
                    ...
                    @if ($tipo_publicacion_edit == 'Comunicados' || $tipo_publicacion_edit == 'Convocatoria')
                        ...
                        <div class="mb-3 col-lg-12">
                            ...
                            <input type="file" class="form-control @error('imagen_edit_nuevo') border-danger @enderror" wire:model="imagen_edit_nuevo"
                                accept=".jpg, .jpeg, .png,.pdf">
                    ...
                    @elseif($tipo_publicacion_edit == 'Convocatoria')
                        <div class="mb-3 col-lg-6">
                            <label class="form-label" for="fecha_inicio_convocatoria_edit">FECHA INICIO DE INSCRIPCIONES CONVOCATORIA</label>
                            <input type="date" class="form-control @error('fecha_inicio_convocatoria_edit') border-danger @enderror" wire:model="fecha_inicio_convocatoria_edit" id="fecha_inicio_convocatoria_edit">
                            @error('fecha_inicio_convocatoria_edit')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="mb-3 col-lg-6">
                            <label class="form-label" for="fecha_fin_convocatoria_edit">CIERRE DE INSCRIPCIONES CONVOCATORIA:</label>
                            <input type="date" class="form-control @error('fecha_fin_convocatoria_edit') border-danger @enderror" wire:model="fecha_fin_convocatoria_edit" id="fecha_fin_convocatoria_edit">
                    ...
                    @elseif($tipo_publicacion_edit == 'Video')
                        <div class="mb-3 col-lg-12">
                            ...
                            <input type="text" class="form-control @error('url_video_edit') border-danger @enderror" wire:model="url_video_edit" id="url_video_edit">
